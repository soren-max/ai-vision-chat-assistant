    ...
    <ReactMarkdown
      components={{
        code({ className, children, ...props }) {
          ...
          if (isInline) {
            return (
              <code className="px-1.5 py-0.5 rounded-sm bg-black/30 text-accent-cyan text-[12px] font-mono" {...props}>
                {children}
              </code>
            );
          }
        ...
        blockquote({ children }) {
          return <blockquote className="border-l-2 border-brand-500/30 pl-3 my-2 text-gray-500 italic">{children}</blockquote>;
        },
        a({ href, children }) {
          return <a href={href} target="_blank" rel="noopener" className="text-accent-blue hover:underline">{children}</a>;
        },
      }}
    >
      {content}
...
function ThinkingBlock({ message }: { message: Message }) {
  ...
  return (
    <div className="flex justify-start animate-fade-in">
      <div className="max-w-[80%] rounded-md overflow-hidden border border-accent-purple/20 bg-accent-purple/5">
        <button
          onClick={() => setCollapsed(!collapsed)}
          className="w-full flex items-center gap-2 px-3 py-2 text-[12px] font-mono text-accent-purple hover:bg-accent-purple/5 transition-colors"
        >
...
function ToolCallBlock({ message }: { message: Message }) {
  ...
  return (
    <div className="flex justify-start animate-fade-in">
      <div className="max-w-[80%] rounded-md overflow-hidden border border-accent-orange/20 bg-accent-orange/5">
        <button
          onClick={() => setCollapsed(!collapsed)}
          className="w-full flex items-center gap-2 px-3 py-2 text-[12px] font-mono text-accent-orange hover:bg-accent-orange/5 transition-colors"
        >
          <svg className={`w-3 h-3 transition-transform ${collapsed ? '' : 'rotate-90'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
          </svg>
...
function MessageBubble({ message }: { message: Message }) {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} animate-fade-in`}>
      {/* AI Avatar */}
      {!isUser && (
        <div className="w-7 h-7 rounded-sm bg-brand-600 flex items-center justify-center shrink-0 mr-3 mt-1">
      ...
      <div className={`max-w-[75%] ${isUser ? 'order-first' : ''}`}>
        <div className={`
          ...
          ${isUser
            ? 'bg-brand-600/20 border border-brand-500/20 text-gray-200 rounded-br-sm'
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="shrink-0 px-5 pb-4 pt-2">
        <div className="flex items-end gap-2 p-2.5 bg-surface-raised border border-surface-border rounded-md focus-within:border-brand-500/30 transition-colors">
          <button className="p-1.5 text-gray-600 hover:text-accent-green transition-colors" title="Voice input">
          ...
          <div className="flex items-center gap-1">
            <button
              onClick={send}
              disabled={!input.trim()}
              className="p-1.5 rounded-sm bg-brand-600 text-white hover:bg-brand-500
