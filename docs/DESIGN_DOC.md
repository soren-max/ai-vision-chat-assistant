...
| 需求 | 完成状态 | 当前实现 |
|------|----------|----------|
| 打开摄像头 | ✅ 已完成 | 首页右上角 `Camera` 面板调用 `getUserMedia`，显示实时视频流 |
| AI 看到视频内容 | ✅ 已完成 | 摄像头启动后每 8 秒抽帧调用 `/api/vision/analyze`，也支持手动 `Analyze` 刷新关键帧 |
| 打开麦克风 | ✅ 已完成 | 输入栏麦克风按钮调用 `useAudioRecorder`，有录音状态、音量反馈和 VAD 自动停止 |
| AI 听到用户说话 | ✅ 已完成 | 录音结束后上传到 `/api/stt`，拿到转写文本后自动进入对话 |
| 视觉+语音联合回答 | ✅ 已完成 | `/api/multimodal/chat` 接收 `user_text + vision_context`，回复会进入聊天流 |
| AI 语音回复 | ✅ 已完成 | 回复后调用 `/api/tts` 自动播报，可在输入栏关闭自动播报 |
| 交互流畅度 | ✅ 已改进 | 去掉首页固定 mock 场景轮播，改为设备状态和云端结果驱动；侧栏展示真实视觉上下文和 Agent 阶段 |
| 成本控制 | ✅ 已采用 | 8 秒视觉抽帧、对话复用最近视觉结果、手动刷新关键帧、STT 按需调用、TTS 仅回复后调用 |
| 运行前提 | ⚠️ 依赖配置 | 需要后端服务运行，并配置 DeepSeek Vision/Chat、STT 相关 API Key；未配置时前端会显示错误反馈 |

### 本次交互改进

1. 摄像头面板从“只展示视频”升级为“视频 + 分析状态 + 最近分析时间 + 手动刷新”。
2. 聊天输入从“文字输入为主”升级为“文字/语音统一入口”：语音转写后自动发送，用户不用复制文本。
3. 对话请求改为优先走多模态接口，将最近一次视觉分析压缩成结构化上下文传给 AI。
4. 回复后自动 TTS 播报，并提供 `voice on/off` 控制，避免每次回复都强制播放。
5. Vision/Agent 侧栏去掉静态示例，改为当前画面、识别物体、风险提示、Agent 阶段与成本策略。
...
| # | 策略 | 是否采用 | 实现方式 |
|---|------|---------|---------|
| 1 | 图片抽帧（降低 Vision 调用频率） | ✅ 已采用 | 前端 8s 自动抽帧 + 手动关键帧刷新 |
...
- **视觉按节奏抽帧**：前端只在摄像头开启后每 8 秒调用一次 `/api/vision/analyze`，避免按视频帧率请求视觉模型。
- **对话复用视觉上下文**：用户连续追问时复用最近一次视觉分析结果，不在每条消息前强制重新分析图像。
- **手动关键帧刷新**：当用户移动物体或切换场景时，可以点 `Analyze` 立即刷新，比持续高频分析更省。
- **STT 按需调用**：只有用户录音结束后才上传音频，VAD 静音停止减少无效音频时长。
- **TTS 可关闭**：语音播报默认开启，但用户可关闭，避免长对话持续产生合成请求。
- **上下文压缩**：传给多模态对话的是摘要、场景、物体、屏幕内容和风险提示，而不是整张图像或完整视觉 JSON。
